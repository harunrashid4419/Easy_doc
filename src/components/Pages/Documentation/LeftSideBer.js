...
const LeftSideBer = () => {
    ...
    return (
        ...
        <div className='bg-gray-100 md:h-screen md:border-r-4 border-gray-200 overflow-y-scroll md:sticky top-0 p-2 hidden md:block md:col-span-3 lg:col-span-1'>
            ...
            <ul className="space-y-2 text-[1.2rem]">
                <li onClick={() => setInstallation(!installation)}
                ...
                >
                    Installation<FaAngleRight className={`inline text-2xl text-gray-600 ${installation ? 'rotate-90' : ''} transition delay-50`}></FaAngleRight>
                </li>
                <li>
                    {
                ...
                <li>
                    {
                        data && data.filter(item => item?.category === 'interview').map(interview => <Link
                            key={interview._id}
                            className={`pl-4 ${interviewOpen ? 'block' : 'hidden'} hover:bg-gray-300 cursor-pointer p-1`} to={`/documentation/${interview?.id}`}
                        >{interview?.title}
                        </Link>
                        )
                    }
                ...
                <li>
                    {
                        data && data.filter(item => item?.category === 'error').map(error => <Link
                            key={error._id}
                            className={`pl-4 ${errorDocOpen ? 'block' : 'hidden'} hover:bg-gray-300 cursor-pointer p-1`} to={`/documentation/${error?.id}`}
                        >{error?.title}
                        </Link>
                        )
                    }
