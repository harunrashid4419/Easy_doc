    ...
    <div className="space-y-4 md:space-y-10">
      ...
      <div className="space-y-4">
        {allTip &&
          allTip.map((tip, idx) => (
            <ul key={idx}>
              {/\d\./.test(tip) ? (
                <li className="text-xl md:text-2xl font-semibold">{tip}</li>
              ) : (
      ...
      {codes && (
        <div className="mockup-code overflow-x-scroll">
          {codes &&
            codes.map((code, idx) => (
              <pre key={idx} className="text-base-100">
                <code>{code}</code>
              </pre>
            ))}
        </div>
      ...
      <div className="border border-gray-800  md:w-1/2 mx-auto p-2 flex justify-evenly hover:bg-black hover:text-white">
        ...
        >
          Yes
          <HiOutlineThumbUp
            className={`text-xl ${state === "yes" && "text-blue-700"}`}
          ></HiOutlineThumbUp>
        </button>
        <button
          onClick={() => dispatch("no")}
        ...
        >
          No{" "}
          <HiOutlineThumbDown
            className={`text-xl ${state === "no" && "text-blue-700"}`}
          ></HiOutlineThumbDown>
        </button>
      </div>
    </div>
